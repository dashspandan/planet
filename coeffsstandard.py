# Set coeffs for accretion interior and exterior to iceline and migration parameters for given disk parameters

import numpy as np

def getcoeffdisk(p, z_o, z, m_star, m_disk, M_dot_accre, r_out):
...
#	h = h_a #disk aspect ratio
	...
	print 'sigma_g = %.2E*a^(-%d)*e^-t/o where a in AU, others in SI' %(sigma_g_coeff, p)

	#Type I migration a_dot_coeff

	y = (3.-p-1.) #exp factor for migration integration, needed in the code for growth in pebble accretion
#	a_dot_coeff = -y*np.divide(6.e24*sigma_g_coeff*2.8*np.power(G1, 0.5), np.power(m_star_kg, 1.5)*np.power(h, 2.))
	a_dot_coeff = -np.divide(6.e24*sigma_g_coeff*1120.*np.power(G1, 0.5), np.power(m_star_kg, 1.5))

	print 'a^(%.2f)-a_o^(%.2f) = integration of %.2E*%.2f*M(in earth masses)e^-t/o dt' %(y, y, a_dot_coeff, y)
